...
const N8N_WEBHOOK_URL =
  "https://n8n.averonix.org/webhook/scraper-run";
...
export function ScraperPage() {
  ...
  const addLog = useCallback(
    (jobId: string, level: LogEntry["level"], msg: string) => {
      setJobs((prev) =>
        prev.map((j) =>
          j.id === jobId
            ? { ...j, logs: [...j.logs, { ts: new Date(), level, msg }] }
            : j
        )
      );
    },
  ...
  const runScraper = async () => {
    ...
    // Simulated phase logs while waiting for webhook response
    const phases = [
    ...
    try {
      ...
      // fill results for URLs not returned
      const returnedUrls = new Set(results.map((r) => r.url));
      urls.forEach((u) => {
        if (!returnedUrls.has(u)) {
          results.push({ url: u, status: "done", chars: 0 });
        }
      });
      ...
      setJobs((prev) =>
        prev.map((j) =>
          j.id === jobId
            ? { ...j, status: "error", progress: 100, finishedAt: new Date() }
            : j
        )
      );
          ...
          <div className="job-list">
            ...
            {jobs.map((j) => (
              ...
              >
                ...
                <div className="job-item-urls">
                  {j.urls[0]}{j.urls.length > 1 ? ` +${j.urls.length - 1}` : ""}
                </div>
                <div className="job-item-footer">
                  <span
                    ...
                    style={{
                      ...
                      borderColor: statusColor(j.status) + "55",
                    }}
                  >
          ...
          <div className="input-area">
            ...
            <div className="input-footer">
              <div className="url-count">
                <span>{parseUrls(urlInput).length}</span> URL
                {parseUrls(urlInput).length !== 1 ? "s" : ""} detectada
                {parseUrls(urlInput).length !== 1 ? "s" : ""}
                {" · una por línea o separadas por coma"}
              </div>
              <button
                className="run-btn"
                disabled={running || parseUrls(urlInput).length === 0}
                onClick={runScraper}
              >
                {running ? (
            ...
            {!activeJob || activeJob.logs.length === 0 ? (
              <div className="console-empty">
                ...
                <div style={{ marginTop: 6, fontSize: 11 }}>
                  Añade URLs arriba y pulsa EJECUTAR SCRAPING
                </div>
              </div>
            ) : (
        ...
        <aside className="results-panel">
                ...
                <div className="result-row" key={i}>
                  <div className="result-row-top">
                    ...
                    <span
                      ...
                      title={r.url}
                      style={{ color: r.status === "done" ? "var(--blue)" : "var(--red)" }}
                    >
                      {r.url}
                    </span>
                  </div>
                  {r.status === "done" && r.chars !== undefined && (
                    <div className="result-chars">
                      {r.chars.toLocaleString()} chars extraídos
                    </div>
                  )}
          ...
          {activeJob && (
            <div className="stats-bar">
              <div className="stat">
                <div className="stat-val">{activeJob.urls.length}</div>
                <div className="stat-lbl">URLs</div>
              </div>
              <div className="stat">
                <div className="stat-val" style={{ color: "var(--green)" }}>
                  {activeJob.results.filter((r) => r.status === "done").length}
                </div>
